...
**Server:** openchargemap-mcp-server
**Version:** 0.1.9
...
const ServerConfigSchema = z.object({
  apiKey: z.string().min(1).describe('Open Charge Map API key (X-API-Key header).'),
  baseUrl: z.string().url().default('https://api.openchargemap.io/v3').describe('OCM API base URL.'),
  referenceRefresh: z
    .stringbool()
    .default(false)
    .describe('Refresh reference data from the live OCM endpoint at startup (bundled-snapshot fallback).'),
});
...
src/
  ...
  data/
    ocm-reference-data.ts               # Bundled OCM reference snapshot (offline ID resolution)
  services/
    openchargemap/
      openchargemap-service.ts          # OCM POI API client (retry + session cache)
      attribution.ts                    # CC BY 4.0 attribution + reliability-note helper
      types.ts                          # Domain types
    reference-data/
      reference-data-service.ts         # Snapshot load, lookup indices, curated aliases, optional refresh
      types.ts                          # Domain types
  mcp-server/
    tools/definitions/
      _station-schema.ts                # Shared station/connection/comment schema + renderers
      *.tool.ts                         # Tool definitions (find / get / lookup / comments)
    resources/definitions/
      station.resource.ts               # openchargemap://station/{id}
```

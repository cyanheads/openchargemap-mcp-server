...
# ==============================================================================
# Pinned to the build host's own platform: `bun run build` emits platform-independent
# JavaScript and only `dist/` crosses into the production stage, which runs its own
# target-arch install. Without this, the linux/amd64 leg of a multi-arch build compiles
# under QEMU, where bun >= 1.4 aborts in the JavaScriptCore allocator (mcp-ts-core#370).
# A build stage that ever compiles a native addon could not cross-compile this way.
...
ARG APP_VERSION
LABEL org.opencontainers.image.title="openchargemap-mcp-server"
LABEL org.opencontainers.image.description="MCP server for the global Open Charge Map EV charging registry: find stations by location and connector, get full station detail, resolve reference IDs, and read community reliability check-ins."
LABEL org.opencontainers.image.licenses="Apache-2.0"
LABEL org.opencontainers.image.version="${APP_VERSION}"
LABEL org.opencontainers.image.source="https://github.com/cyanheads/openchargemap-mcp-server"
...
RUN mkdir -p /var/log/openchargemap-mcp-server && chown -R bun:bun /var/log/openchargemap-mcp-server
...
ENV LOGS_DIR="/var/log/openchargemap-mcp-server"

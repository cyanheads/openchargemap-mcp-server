...
Thanks for using `openchargemap-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

Open one from the **Issues** tab and pick the [**Bug Report**](https://github.com/cyanheads/openchargemap-mcp-server/issues/new?template=bug_report.yml) or [**Feature Request**](https://github.com/cyanheads/openchargemap-mcp-server/issues/new?template=feature_request.yml) form. Both are structured, and filling in the fields is what makes an issue actionable.

Pull requests are welcome; open an issue first for anything larger than a typo. Run `bun run devcheck` and `bun run test` before submitting.

## Server bug or framework bug?

`openchargemap-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:
...
Station data itself comes from [Open Charge Map](https://openchargemap.org). A wrong address, a missing connector, or a stale operational status is a registry record, not a server bug — correct it at the source and this server will serve the correction.
...
3. **Redact anything sensitive.** Issues are public and permanent — no API keys, tokens, auth headers, internal URLs, or PII in code, logs, or stack traces. `OPENCHARGEMAP_API_KEY` in particular rides the `X-API-Key` header and shows up in verbose request logs.
